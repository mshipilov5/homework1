...
double calculating_area_rectangle_rule(double left_border, double right_border) { 
	double area_of_recangle = (right_border - left_border) * sin((left_border + right_border) / 2);
	return area_of_recangle;
} 

double calculating_area_simpsons_rule(double left_border, double right_border) {
	double area_of_parabolic_trapezoid = (right_border - left_border) / 6 * (sin(left_border) + 4 * sin((left_border + right_border) / 2) + sin(right_border));
	return area_of_parabolic_trapezoid;
}
...
char **calculate_integrals(struct interval_t interval, unsigned int count, const unsigned int *partition_sizes) {
	...
	for (unsigned int i = 0; i < count; ++i) {
		double integral_rectangle = integrate(interval, partition_sizes[i], calculating_area_rectangle_rule);
		double integral_simpson = integrate(interval, partition_sizes[i], calculating_area_simpsons_rule);
